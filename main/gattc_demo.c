...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        ...
        break;
    case ESP_GATTC_READ_CHAR_EVT: // Successfully read characteristic
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        vTaskDelay(100);
        //try reconnecting
        ESP_LOGI(GATTC_TAG, "TRYING TO RE-ESTABLISH CONNECTION.");
        esp_err_t ret;
        ret = esp_ble_gap_set_scan_params(&ble_scan_params);
        if (ret)
        {
            ESP_LOGE(GATTC_TAG, "%s esp_ble_gap_set_scan_params failed, error code = %x\n", __func__, ret);
            return;
        }
        break;
    default:
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT:
    {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            payload = scan_result->scan_rst.ble_adv;
            while ((payload - scan_result->scan_rst.ble_adv) < 31)
            {
                length = *payload;
                payload++;
                switch (*payload)
                { // Check Payload type
                case 0x00:
                    ESP_LOGI(GATTC_TAG, "NO PAYLOAD.");
                    break;
                case ESP_BLE_AD_TYPE_16SRV_CMPL: // Complete list of 16-bit serivce UUIDS
                    ESP_LOGI(GATTC_TAG, "COMPLETE LIST OF UUIDS.");
                    uuidPtr = payload + 1;
                    esp_log_buffer_hex(GATTC_TAG, uuidPtr, length - 1);
                    if (length - 1 == 2)
                    {
                        //create UUID struct
                        found_uuid = (esp_bt_uuid_t){
                            .len = ESP_UUID_LEN_16,
                            .uuid = {
                                .uuid16 = (((uint16_t)uuidPtr[1] << 8) | uuidPtr[0]),
                            },
                        };
                    }
                    break;
                case ESP_BLE_AD_TYPE_NAME_CMPL: //Complete local name - Bluetooth Core Specification:Vol. 3, Part C, section 8.1.2 (v2.1 + EDR, 3.0 + HS and 4.0)Vol. 3, Part C, sections 11.1.2 and 18.4 (v4.0)Core Specification Supplement, Part A, section 1.2
                    ESP_LOGI(GATTC_TAG, "# Complete local name: %.*s", length, payload);
                    break;
                default:
                    ESP_LOGI(GATTC_TAG, "Unknown Type: %d", *payload);
                    break;
                }
                payload += length;
            }
            ...
            ESP_LOGI(GATTC_TAG, "\n");
            if (found_uuid.len == ESP_UUID_LEN_16 && found_uuid.uuid.uuid16 == REMOTE_SERVICE_UUID)
            {
